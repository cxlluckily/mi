<template>
    <div id='OutboundAddParts'>
        <div class="OutboundAddParts-main">
            <div class="OutboundAddParts-from">
            ...
            <div class="OutboundAddParts-list">
                            ...
                            <div class="list-details">
                                ...
                                <div class="details-content">
                                    ...
                                    <div class="details-status">
                                        <p><span>备件状态 :</span><span>{{itemList.status=='normal'?'好件':'坏件'}}</span></p>
                                    </div>
...
export default {
  ...
  methods: {
    /* 方法 */
    initData(paramsObj) {
      this.api.addPartsDataAllprompObj(paramsObj).then(data => {
    ...
    goFather() {
      const ICONDOM = $(".iconSelected"),
        ICONDOMARR = [],
        SPAREPARTIDARR = this.paramsObj.sparePartId.split(","),
        SPAREPARTIDCOUNTARR = this.paramsObj.sparePartIdCount.split(","),
        COUNTNUM = (arr, value) =>
          arr.reduce((a, v) => (v === value ? a + 1 : a + 0), 0); // 获取数组中某个值出现的次数
      let addID = 0,
        addCount = 0;
      // 获取 选中的 id
      for (let i = 0, II = ICONDOM.length; i < II; i++) {
        ICONDOMARR.push(ICONDOM.eq(i).attr("data-sparePartId"));
      }
      // 比对数据，看是否大于库存以后值
      // for (let v = 0, VV = SPAREPARTIDARR.length; v < VV; v++) {
      //   console.log(COUNTNUM(ICONDOMARR, SPAREPARTIDARR[v]));
      //   if (COUNTNUM(ICONDOMARR, SPAREPARTIDARR[v]) > SPAREPARTIDCOUNTARR[v]) {
      //     this.EquipmentRepairErrorTitle = "件数错误";
      //     this.EquipmentRepairError = "超出了库存已有值!";
      ...
      this.$router.replace({
        ...
        query: {
          ...
          outStockApplyId: this.paramsObj.tempOutStockApplyId,
          operationSubjectId: this.paramsObj.tempOperationSubjectId
        }
      });
    },
  ...
  beforeRouteLeave(to, from, next) {
    if (to.path == "/Outbound" && from.path == "/OutboundAddParts") {
      next({
        path: "/OutboundThrow"
      });
    } else {
  ...
  mounted: function() {
    /* 初始化数据 */
    this.paramsObj = this.$route.query.params;
    if (this.$route.query.checkedArr) {
      this.checkedNames = this.$route.query.checkedArr;
    }
